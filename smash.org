...
:PROPERTIES:
:EXPORT_FILE_NAME: classification
:EXPORT_METADATA: (("git" ("suppress_outputs" . t)))
:EXPORT_TITLE: Classification of Characters in Super Smash Bros. Melee
:END:
#+SESSION: smash
...
:END:
For interested parties, the following describes the code used to generate the custom images.  Not all the code will be covered, in particular I assume the reader is familiar with [[https://www.gnu.org/software/make/manual/make.html][GNU Make]] and can read the [[file:Makefile][Makefile]] included in this repository.
...
If you generated image and mask data according to [[#Data-Generation][the documentation]] above, copy all images into the =data/images= subfolder and copy all masks into the =data/masks= subfolder.\\
\\
...
:END:
This repository contains my notes and work applying neural networks to footage from the [[https://en.wikipedia.org/wiki/Super_Smash_Bros._Melee][Super Smash Bros. Melee]] video game.

Currently, I have used transfer learning techniques to train a VGG16-based neural network to classify images of single characters.  See the [[file:classification.ipynb][classification notebook]] for details.
