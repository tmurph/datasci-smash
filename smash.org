...
#+METADATA: (("git" ("suppress_output" . t)))
...
***** Training Data
First, let’s have a look at the images used to train the CNN.  Over 100,000 images were used, so we’ll draw a random sample and use [[https://matplotlib.org/][Matplotlib]] to display them.
...
These images were all captured from scripted gameplay.  The [[file:smash.org::*Data%20Generation][Data Generation]] section includes technical details on the capturing process.

Let’s compare these images against the images used in the ImageNet competition to train VGG16.  E.g. [[http://imagenet.stanford.edu/synset?wnid=n02123394][these images from the persian cat category]].
...
To compare true classification labels with the predictions our model emits, we’ll need to iterate twice over our test images in deterministic order again.  One pass will run the images through the model to produce predicted labels, and the other will collect the true labels for the images.
...
By default our class labels are encoded as integers, but we can actually find the actual class names as taken from the folder hierarchy if we know where to look.
