...
:PROPERTIES:
:header-args:jupyter+: :existing kernel-b7e341bd-9c5e-46a1-beaa-b082d4449c2a.json
...
: /run/user/1000/jupyter/kernel-b7e341bd-9c5e-46a1-beaa-b082d4449c2a.json
...
**** TODO Review of Transfer Learning
I assume the reader is already familiar with convolutional neural networks (CNN), the Python language, and the OpenCV, Tensorflow, and Keras libraries.

Many practical applications of neural networks to computer vision tasks require very deep CNNs that can be prohibitively expensive to train.  However, we are fortunate that researchers with access to extremely powerful hardware have trained deep CNNs and moreover have published their results in a way that allows others to “stand on their shoulders”.

The availability of pre-trained deep CNNs enables so-called “transfer learning”, 

**** Transfer Learning Code

***** Training Data
Let’s have a look at some of the images used to train the CNN.  These images were captured from scripted gameplay.  The [[file:smash.org::*Data%20Generation][Data Generation]] section includes technical details on the capturing process.
...
#+BEGIN_SRC jupyter 
  ...
  def get_image(character, color, stage, orientation, img_number):
      image = os.path.join(
          DATA_DIR, 'train', 'images', character,
          f'{character}_{color}_{stage}_{orientation}_bg_on_{img_number}.jpg')
...
#+BEGIN_SRC jupyter :results file :exports both
  import random
  ...
  for i, image_name in enumerate(random.sample(all_images, NUM_ROWS * NUM_COLS)):
      ...
      match = re.search(img_re, image_name)
      ax.imshow(get_image(**match.groupdict()))

  fig.tight_layout()
#+END_SRC

#+RESULTS:
[[file:./jupyter_images/66007eW.png]]
...
We can wrap these steps in a function and do the same for our validation and test sets.

#+BEGIN_SRC jupyter 
  def get_features_and_labels(folder="train"):
      image_count = count_images(folder)
      image_count_generator_cutoff = image_count / BATCH_SIZE + 1

      image_generator = make_generator(
          folder,
          data_gen_args={"preprocessing_function": preprocess_input},
          data_flow_args={"batch_size": BATCH_SIZE,
                          "class_mode": None,
                          "shuffle": False})
      label_generator = make_generator(
          folder,
          data_gen_args={},
          data_flow_args={"batch_size": BATCH_SIZE,
                          "shuffle": False})
      ...
      labels = []
      for i, (_, l) in enumerate(label_generator):
          if i >= image_count_generator_cutoff:
              break
          else:
              labels.extend(l)
      labels = np.array(labels[:image_count])

      feature_data = vgg_model.predict_generator(image_generator,
                                                 steps=image_count_generator_cutoff)
      feature_data = feature_data[:image_count]

      return feature_data, labels

  valid_data, valid_labels = get_features_and_labels("valid")
  test_data, test_labels = get_features_and_labels("test")
#+END_SRC
...
#+BEGIN_SRC jupyter 
  ...
  test_data = test_data[test_randomizer]
  test_labels = test_labels[test_randomizer]
#+END_SRC
...
Per a bug in Keras, we can’t actually save the =compiled_model= to disk.  However, all the structure and learned weights of the =compiled_model= are actually shared with the =template_model= object, so we can just save that.

#+BEGIN_SRC jupyter 
  import datetime

  template_model.save(os.path.join(DATA_DIR, datetime.datetime.today().strftime('%Y-%m-%d-%H-%M') + "-model.h5"))
#+END_SRC
...
#+BEGIN_SRC jupyter 
  ...
  true_classes = test_labels.argmax(1)
  pred_classes = compiled_model.predict(test_data).argmax(1)
...
#+BEGIN_SRC jupyter :results file
  class_names = [c for c, i in sorted(generator_train.class_indices.items(), key=lambda pair: pair[1])]

  plot_confusion_matrix(c_matrix, class_names, normalize=True)
#+END_SRC

**** TODO Data Generation
Talk about your code here
